...
infl_data <- data_wg %>%
  ...
  #take out date and inflaiton 
  dplyr::select(date, infl) %>%                      
  #remove missing values
  filter(!is.na(infl)) %>%                    
  #lagged inflation
  mutate(infl_lag1 = lag(infl, n = 1)) %>%   
  filter(!is.na(infl_lag1))
head(infl_data)
...
ar1_est <- function(data_window) {
  ...
  # Estimate AR(1) model: infl_t = α + ρ * infl_{t-1} + ε_t
  model <- lm(infl ~ infl_lag1, data = data_window)
  ...
  se_alpha <- std_errors["(Intercept)"]
  se_rho <- std_errors["infl_lag1"]
...
for (i in 1:(nrow(infl_data) - window_size + 1)) {
  # Extract window of data
  window_data <- infl_data[i:(i + window_size - 1), ]
  
    window_results <- ar1_est(window_data)
  
  # Add end date of the window
  window_results$date <- window_data$date[window_size]
  
  # Add to results
  rolling_results <- rbind(rolling_results, window_results)
}

head(rolling_results)
...
p_rho <- ggplot(rolling_results, aes(x = date, y = rho)) +
  geom_line(color = "steelblue", linewidth = 1) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  geom_hline(yintercept = 1, linetype = "dashed", color = "red", alpha = 0.5) +
  labs(
    title = "Rolling Estimates of Inflation Persistence (ρ)",
    subtitle = "10-year rolling window AR(1) model",
    x = "Date (End of Window)",
    y = "Persistence Parameter (ρ)"
  ) +
