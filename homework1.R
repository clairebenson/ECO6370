...
library(stats)
library(MASS)
#library(ggplot) - probably don't include, don't want to make charts using this library/functions 
``` 
...
bhat <- qr.solve(t(X) %*% X, t(X) %*% y)        #estimated coefficients (not the "true" values)
yhat <- X %*% bhat                              #estimated value of the dependent variable (regression line)
residuals <- y-yhat
...
sighat2 = sum(residuals^2)/df                    #unbiased estimator of the variance of the error term in a linear regression model
